...
class Pileup
  ...
  def self.read_bases_to_hash(pileup)
    ...
    # if pileup bases hash have Ns check how they influence coverage
    # if bases_hash.key?(:N) and bases_hash[:N] > 0
    #   warn "ambiguous read bases at this position\t#{bases_hash}"
    # end
    bases_hash
  end
