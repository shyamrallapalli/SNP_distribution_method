...
class TestFragments < Test::Unit::TestCase
  ...
  def test_split
    ...
    right, left, inkeys = Fragments.split(@dic_hm_inv, right = [], left = [], inkeys, 0)
  ...
  def test_sort
    perm_back, mut_back = Fragments.sort(@dic_hm_inv, cross = 'back', average_contig = 2500)
    assert_equal(['e', 'd', 'a', 'b', 'c', 'f'], perm_back)
    assert_equal(['e', 'd', 'a', 'b', 'c', 'f'], mut_back)
  end

  def test_arrange
    perm_hm, perm_ratio, mut, hyp = Fragments.arrange(@dic_hm_inv, @dic_ratio_inv, @dic_pos_hm, cross = 'back', average_contig = 2500)
